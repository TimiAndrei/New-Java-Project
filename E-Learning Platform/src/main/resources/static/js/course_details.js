  ...
  div.innerHTML = `
    <div class="card-body">
      ...
      <div class="mb-2"><label>Question</label>
        <input type="text" class="form-control edit-question-text" value="${
          q.text ?? ""
        }" required></div>
      <div class="row">
        <div class="col-md-6 mb-2"><label>A</label>
          <input type="text" class="form-control edit-option1" value="${
            q.option1 ?? ""
          }" required></div>
        <div class="col-md-6 mb-2"><label>B</label>
          <input type="text" class="form-control edit-option2" value="${
            q.option2 ?? ""
          }" required></div>
        <div class="col-md-6 mb-2"><label>C</label>
          <input type="text" class="form-control edit-option3" value="${
            q.option3 ?? ""
          }" required></div>
        <div class="col-md-6 mb-2"><label>D</label>
          <input type="text" class="form-control edit-option4" value="${
            q.option4 ?? ""
          }" required></div>
      </div>
      <div class="mb-2">
        ...
        <select class="form-select edit-correct-option">
          <option value="0" ${
            q.correctOptionIndex === 0 ? "selected" : ""
          }>A</option>
          <option value="1" ${
            q.correctOptionIndex === 1 ? "selected" : ""
          }>B</option>
          <option value="2" ${
            q.correctOptionIndex === 2 ? "selected" : ""
          }>C</option>
          <option value="3" ${
            q.correctOptionIndex === 3 ? "selected" : ""
          }>D</option>
        </select>
      </div>
...
if (document.getElementById("editQuizForm")) {
  document.getElementById("editQuizForm").onsubmit = function (e) {
    ...
    const questionBlocks = document.querySelectorAll(
      "#editQuizQuestionsContainer .edit-question-block"
    );
    const questions = [];
    questionBlocks.forEach((block) => {
      ...
      const correctOptionIndex = parseInt(
        block.querySelector(".edit-correct-option").value
      );
      questions.push({
        id,
        text,
  ...
  document
    ...
    .addEventListener("submit", function (e) {
      ...
      const courseId = document
        .getElementById("courseDetailsContainer")
